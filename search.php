...
if (isset($_GET['name'])) {
    $product_list = $db->getAll("select * from product where name like '%{$_GET['name']}%'");
    ...
    $list = $db->getById("catalog", $catalog_id);
    $sql_product = "select * from product where true";

    if ($list['parent_id'] == '1') {
        $list_child = $db->getAll("select * from catalog where parent_id = $catalog_id");
        $list_id = array();
        foreach ($list_child as $value) {
            $list_id[] = $value['id'];
        }
        $str_list = implode(",", $list_id);
        $sql_product .=  " and catalog_id in ($str_list)";
    } else {
        $sql_product .=  " and catalog_id = $catalog_id";
    }
    $sql_product .= " and price >= $price_from and price <= $price_to order by price asc";
